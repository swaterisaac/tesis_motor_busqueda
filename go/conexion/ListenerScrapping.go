...
func ListenerGeneral(dbApp *sql.DB, dbScrapping *sql.DB, funcionQuery func(int) string, funcionTraductor traductor, nombreDBScrapping string) {
	...
	if !existeTabla {
		...
		if err != nil {
			log.Fatal("Hay un error al crear la nueva tupla\nError: ", err)
		}
	...
	if err != nil {
		log.Fatal("Error al obtener el valor de última tupla\nError: ", err)
	}
	...
	if err != nil {
		log.Fatal("Hay un error obteniendo las últimas ofertas\nError: ", err)
	}
	...
	for _, oferta := range ofertas {
		...
		filas.Close()
		if err != nil {
			log.Fatal("Hay un error evaluando si la región de la oferta ya existe\nError: ", err)
		}
		...
		if empty {
			...
			if err != nil {
				log.Fatal("Hay un error creando la nueva región\nError: ", err)
			}
		...
		if err != nil {
			log.Fatal("Hay un error evaluando si la comuna de la oferta ya existe\nError: ", err)
		}
		empty = !filas.Next()
		filas.Scan(&idComuna)
		filas.Close()
		...
		if empty {
			...
			if err != nil {
				log.Fatal("Hay un error creando la nueva comuna\nError: ", err)
			}
		...
		if err != nil {
			log.Fatal("Hay un error evaluando si el proveedor de la oferta ya existe\nError: ", err)
		}
		empty = !filas.Next()
		...
		if err != nil {
			log.Fatal("Hubo un error creando la nueva oferta turística\nError: ", err)
		}
